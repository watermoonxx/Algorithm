...
### 문제 리뷰
> 처음 풀 때는 딕셔너리 생성 후 key에 해당하는 value 값에 따라서 연산을 진행했다.<br/>
딕셔너리는 순회 시 순서가 보장되지 않지만 key와 value가 각각 연결되었으니 연산 순서는 상관없을거라 생각했는데 계속 오답이었다.<br/>
zip() 함수로 리스트끼리 묶어서 튜플의 형태로 만드는 과정까지만 진행하고, 각각의 원소에 접근해서 연산했다.
<br/>
